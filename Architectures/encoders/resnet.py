...
class ResNetEncoder(ResNet, EncoderMixin):
    def __init__(self, out_channels, depth=5, **kwargs):
        ...
        self.unfold_layer_up = nn.Sequential(
            nn.Conv2d(3, 1, kernel_size=(1,1), stride=1),
            nn.Unfold(kernel_size=(self.patch_size, self.patch_size), stride=(self.patch_size,self.patch_size))
        )

        self.unfold_layer_down = nn.Sequential(
            nn.Conv2d(64, 1, kernel_size=(1,1), stride=1),
            nn.Unfold(kernel_size=(self.patch_size, self.patch_size), stride=(self.patch_size,self.patch_size))
        )
    ...
    def forward(self, x):
        #att_activation = torch.nn.Sigmoid()
        att_activation = torch.nn.ReLU()
        ...
        patch_1 = patch_1.transpose(0, 2).transpose(1, 2)

        # print(patch_1.size())
        # torch.Size([1024, 4, 100])
        ...
        patch_2 = patch_2.transpose(0, 2).transpose(1, 2)
        fold_layer_down = torch.nn.Fold(output_size=(x.size()[-2], x.size()[-1]), kernel_size=(self.patch_size, self.patch_size), stride=(self.patch_size, self.patch_size))

        patch1_to_patch2 = self.transformer_layer_down(patch_1, patch_2)
        patch2_to_patch1 = self.transformer_layer_up(patch_2, patch_1)

        patch2_to_patch1 = patch2_to_patch1.transpose(0,1)
        patch1_to_patch2 = patch1_to_patch2.transpose(0,1)
        # print(patch1_to_patch2.size())
        # print(patch2_to_patch1.size())
        # torch.Size([4, 64, 100])
        # torch.Size([4, 1024, 100])
        ...
        att_patch_1=torch.bmm(patch2_to_patch1, patch1_to_patch2.transpose(1, 2))

        att_patch_1 = self.linear_layer_1(att_patch_1)
